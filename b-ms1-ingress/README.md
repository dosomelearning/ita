...
- `MS1` no longer receives the shared processing bucket name or `MS4` API base URL as manually duplicated values in `samconfig.toml`.
- `MS1` imports shared infra values from `b-infra` and the internal API base URL from the owning `MS4` stack.
- This removes environment-specific hardcoding of bucket names, SPA origin, and state-manager API URL from service deployment overrides.

`MS1` also exports its own API endpoint as `${AWS::StackName}-ApiEndpoint`.
